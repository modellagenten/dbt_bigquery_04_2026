...
-- Verkaufskennzahlen, wie Gesamtanzahl der verkauften Produkte, Gesamterlös, Erstes
-- und Letztes
-- verkauftes Produkt.
select
...
join
    {{ ref("stg_order_items") }} on stg_products.product_id = stg_order_items.product_id
group by
    stg_products.product_id,
    stg_products.product_name,
    stg_products.brand,
